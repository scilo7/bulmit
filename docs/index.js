...
var layout = {
    view: function (vnode) { return [
        mithril('section.hero.is-dark', 
            mithril('hero-body',
                mithril('.container', 
                    mithril(Title, {size: 1}, 'Bulmit'),
                    mithril(SubTitle, {size: 4}, mithril.trust('[Bulma + Mithril] = &#9825;'))
                )
            ),
            mithril('.hero-foot',
                mithril('.container', mithril(Nav, {
                    left: [
                        mithril('a[href=#/api]', 'api'),
                        mithril('a[href=#/api]', 'demo') ],
                    right: [
                        mithril('a[href=https://github.com/scilo7/bulmit]', mithril(Icon, {icon: 'github'}))
                    ]
                }))
            )
        ),
        mithril('.container', vnode.children)
    ]; }
};

var App = {
    view: function () { return mithril(layout, 'hello world'); }
};
